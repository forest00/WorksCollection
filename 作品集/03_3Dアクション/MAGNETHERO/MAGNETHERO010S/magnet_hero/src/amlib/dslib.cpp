...
static LPDIRECTSOUND pDSound = nullptr;
...
static CHANNELSTRUCT *pChannel = nullptr;
...
BOOL DSoundInit(
	...
	LPGUID lpGuid		// 使うデバイスのGUID、nullptrだとデフォルト
) {	
	// オブジェクトを生成する
	if ( DirectSoundCreate( lpGuid, &pDSound, nullptr ) != DS_OK ) {
		return FALSE;
	}
...
//	pDSound ->SetSpeakerConfig(DSSPEAKER_HEADPHONE);
	...
	dsdc.lpwfxFormat	= nullptr;
	pDSound->CreateSoundBuffer( &dsdc, &pDSPrimary, nullptr) ;
...
BOOL DSoundRelease() {
	...
	// DirectSoundの解放
	pDSPrimary->Release(); pDSPrimary = nullptr;
	pDSound->Release(); pDSound = nullptr;

	// チャンネルテーブルを解放
	free( pChannel ); pChannel = nullptr;
...
static BOOL CreateBuffer( int nChannel,int iHeletz,int iBit,int iMSFlg, DWORD dwSize ) {
	...
	if ( pDSound->CreateSoundBuffer( &dsbdesc, &pChannel[nChannel].lpBuffer, nullptr ) != DS_OK ) {
		return FALSE;
	}
...
static BOOL DSoundLoadFileStatic( LPSTR szFileName, int nChannel ) {
	...
	// ファイルのオープン
	hmfr = mmioOpen(szFileName, nullptr, MMIO_READ | MMIO_ALLOCBUF );
	if ( hmfr == nullptr ) {
		// ファイルオープン失敗
		return FALSE;
	}
	...
	if ( mmioDescend( hmfr, &parent, nullptr, MMIO_FINDRIFF ) ) {
		// RIFFディセンド失敗
		mmioClose( hmfr, 0 );
		return FALSE;
	}
...
static unsigned __stdcall thStreamPlay( LPVOID lpParam ) {
	...
	while( !bEndFlg ) {
		...
		if( ev == WAIT_OBJECT_0 ) {
			...
			// dwPosに現在の再生ポジションを得る
			DWORD dwPos;
			pChannel[nChannel].lpBuffer->GetCurrentPosition( &dwPos, nullptr );
...
int DSoundLoadFile(
	...
	if ( pDSound == nullptr ) { return TRUE; }
	...
	if ( dwFlg == DSLF_FLG_STATIC ) {
		...
		// ストリーム
		// イベント作成
		pChannel[ nChannel ].hEvent = CreateEvent( nullptr, FALSE, FALSE, nullptr );
		...
		// ストリーム再生用スレッドの作成
		unsigned dummy;
		pChannel[ nChannel ].hThread = (HANDLE)_beginthreadex( nullptr, 0, thStreamPlay, (void*)nChannel, 0, &dummy );
...
BOOL DSoundPlay( int nChannel, BOOL bLoop ) {
	...
	if ( pChannel[nChannel].lpBuffer == nullptr ) { return TRUE; }
...
void DSoundStop( int nChannel ) {
	if ( pDSound == nullptr ) { return; }
	if ( nChannel < 0 || nChannel > nChannelMax ) { return; }
	
	if ( pChannel[nChannel].lpBuffer == nullptr ) { return; }
...
BOOL DSoundReleaseChannel( int nChannel ) {
	...
	DSoundStop( nChannel );
	if ( pChannel[nChannel].lpBuffer != nullptr ) {
		if ( pChannel[nChannel].bStream ) {
			pChannel[nChannel].bStream	= FALSE;
			pChannel[nChannel].dwEvent	= DSCTRL_EVENT_EXIT;
			SetEvent( pChannel[nChannel].hEvent );
			WaitForSingleObject( pChannel[nChannel].hThread, 100 );
			CloseHandle( pChannel[nChannel].hThread ); pChannel[nChannel].hThread = nullptr;
			CloseHandle( pChannel[nChannel].hEvent ); pChannel[nChannel].hEvent = nullptr;
		}
		pChannel[nChannel].lpBuffer->Release();
		pChannel[nChannel].lpBuffer = nullptr;
	}
	return TRUE;
}
